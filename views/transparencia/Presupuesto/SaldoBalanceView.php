                    ...
                    foreach ($dataTable as $row) {
                        ...
                        $div =  '<div class="card p-3">
                                    ...
                                    <div class="d-flex mb-3">
                                        ...
                                        <h5 class="fwSemiBold">
                        ';
                        $div .= '<a href="https://regionloreto.gob.pe/files/presupuesto/saldo-balance/'.$row['pathfile'].'" class="card-title" download>'. $row['title'].'</a><h5>';
                        $div .= '<strong class="d-block fileSize font-weight-normal card-text">' . $fecha . '</strong></div>';
                        $div .= '</div><a class="btn btn-outline light btnAlerDark btnNoOver btn-sm" "https://regionloreto.gob.pe/files/presupuesto/saldo-balance/'.$row['pathfile'].'" download>Ver Documento</a></div></div>';
                        echo $div;
                    }
                ?>
            </div>
